...
@views.route('/')
@cross_origin()
# the first thing we see when going to website is the login page
def login():
    try:
...
#after login the dashboard page comes
...
# If the person wants to see their profile ,so make profile fuction to see the profile
@views.route('/profile')
@cross_origin()
def profile():
    try:
